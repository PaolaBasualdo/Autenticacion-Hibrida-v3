...
router.post("/google", async (req, res) => {
  try {
    ...
    // Paso 2: Buscamos al usuario en nuestra base de datos por email
    ...
    if (!usuario) {
      ...
      // Si ya existe pero tiene otro proveedor, lo actualizamos a Google
...
router.get(
  ...
  (req, res) => {
    // Si la autenticación es exitosa, req.user existirá.
    // Si no lo es, este middleware no se ejecutará,
    // y el error se manejará en el siguiente 'catch' o por defecto.
